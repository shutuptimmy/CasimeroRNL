...
const DeleteGenderForm = ({ onDeleteGender }: DeleteGenderFormProps) => {
    ...
    const handleGetGender = (genderID: number) => {
        GenderService.getGender(genderID).then((res) => {
            if (res.status === 200) {
                setState((prevState) => ({
                    ...prevState,
                ...
                console.error("unexpected error while getting genderL: ", res.status)
            }
        }).catch((error) => {
    ...
    const handleDestroyGender = (e: FormEvent) => {
        ...
        GenderService.destroyGender(state.gender_id).then((res) => {
            if (res.status === 200) {
